...
/***********************************************************************************************************************************************************************************************************************/
#include "pCT_Reconstruction_command_line_args.h"
...
//**********************************************************************************************************************************************************************************************************
//bool* intersected_hull, unsigned int* first_MLP_voxel, bool* hull, float* x_entry, float* y_entry, float* z_entry, float* xy_entry_angle, float* xz_entry_angle, float* x_exit, float* y_exit, float* z_exit, float* xy_exit_angle, 
...
//					  xz_exit_angle, WEPL, lambda, i, post_cut_protons, a_i, update_value_history, number_of_intersections);
...
int main(int argc, char** argv)
{
	...
	if( RUN_ON )
	{
		...
		if( WRITE_X ) {
		   char x[] ={"x"};
			array_2_disk(x, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, x_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );}
	}
	else
	{
		//binary_2_ASCII();
...
void initialize_parameters() {
	...
	char in_directory[] = "//home//karbasi//Public//";
	char out_directory[] = "//home//karbasi//Public//";
	char in_folder[] = "input_CTP404_4M";
	char out_folder[] = "cuda_test";
	...
	strcpy (parameter_container.INPUT_DIRECTORY,in_directory);
	strcpy (parameter_container.OUTPUT_DIRECTORY,out_directory);
	strcpy (parameter_container.INPUT_FOLDER,in_folder);
	strcpy (parameter_container.OUTPUT_FOLDER,out_folder);
	...
	///parameter_container.OUTPUT_DIRECTORY  	= "//home//karbasip//";
	///parameter_container.INPUT_FOLDER	 	= "input_CTP404_4M";
	///parameter_container.OUTPUT_FOLDER    	= "cuda_test";
	parameter_container.MAX_GPU_HISTORIES		= static_cast<int>(3200000);
	parameter_container.MAX_CUTS_HISTORIES		= static_cast<int>(3200000);
	parameter_container.LAMBDA 		= 	0.001;
	parameter_container.ITERATIONS			= 12;
	parameter_container.DROP_BLOCK_SIZE			= static_cast<int>(320000);
	parameter_container.THREADS_PER_BLOCK		= static_cast<int>(320);
	parameter_container.ENDPOINTS_PER_BLOCK 		= static_cast<int>(320);
	parameter_container.HISTORIES_PER_BLOCK		= static_cast<int>(320);

	parameter_container.ENDPOINTS_PER_THREAD 		= static_cast<int>(1);
	parameter_container.HISTORIES_PER_THREAD 		= static_cast<int>(1);
	parameter_container.VOXELS_PER_THREAD			= static_cast<int>(1);

	parameter_container.MAX_ENDPOINTS_HISTORIES 		= static_cast<int>(64000000);
...
void command_line_settings( unsigned int num_arguments, char** arguments )
{
	...
	char temp[50];
	//char tt[] = "LAMBDA";
	//strcpy(temp,run_arguments[1]);
	
	if (num_arguments > 1) {
	  
		for(int i = 1 ; i < num_arguments; i+=2) {
		  
			strcpy(temp,run_arguments[i]);
			//  printf("%s\n", temp);
			//  printf("%s\n", tt);
			//  if(strcmp( temp,tt ) == 1)
			//    printf("hi: %s\n",temp);
...
void apply_tuv_shifts( unsigned int num_histories)
{
	...
	if( WRITE_SSD_ANGLES )
	{
		...
		array_2_disk( data_filename, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, ut_entry_angle, COLUMNS, ROWS, SLICES, num_histories, true );
		sprintf(data_filename, "%s_%03d%s", "uv_entry_angle", gantry_angle, ".txt" );
		char ut_entry_angle[] = {"ut_entry_angle"};
		array_2_disk( ut_entry_angle, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, uv_entry_angle, COLUMNS, ROWS, SLICES, num_histories, true );
		sprintf(data_filename, "%s_%03d%s", "ut_exit_angle", gantry_angle, ".txt" );
		array_2_disk( ut_entry_angle, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, ut_exit_angle, COLUMNS, ROWS, SLICES, num_histories, true );
		sprintf(data_filename, "%s_%03d%s", "uv_exit_angle", gantry_angle, ".txt" );
		array_2_disk( ut_entry_angle, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, uv_exit_angle, COLUMNS, ROWS, SLICES, num_histories, true );
	}
}
...
void binning( const int num_histories )
{
	...
	if( WRITE_BIN_WEPLS )
	{
		...
		array_2_disk( data_filename, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, bin_num_h, COLUMNS, ROWS, SLICES, num_histories, true );
	}
...
void calculate_means()
{
	...
	array_2_disk(bin_counts, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, bin_counts_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );
	array_2_disk(mean_wepl, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, mean_WEPL_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );
	array_2_disk(mean_rel_ut, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, mean_rel_ut_angle_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );
	array_2_disk(mean_rel_uv, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, mean_rel_uv_angle_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );
...
void calculate_standard_deviations()
{
	...
	array_2_disk(stddev_rel_ut, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, stddev_rel_ut_angle_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );
	array_2_disk(stddev_rel_uv, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, stddev_rel_uv_angle_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );
	array_2_disk(stddev_wepl, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, stddev_WEPL_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );
...
void construct_sinogram()
{
	...
	array_2_disk( bin_counts_pre, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, bin_counts_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );

	cudaMemcpy(sinogram_h,  sinogram_d, SIZE_BINS_FLOAT, cudaMemcpyDeviceToHost);
	char sinogram_pre[]={"sinogram_pre"};
	array_2_disk(sinogram_pre, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, sinogram_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );
	...
	array_2_disk(sinogram, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, sinogram_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );

	cudaMemcpy(bin_counts_h, bin_counts_d, SIZE_BINS_INT, cudaMemcpyDeviceToHost) ;
	char bin_counts_post[]={"bin_counts_post"};
	array_2_disk( bin_counts_post, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, bin_counts_h, T_BINS, ANGULAR_BINS, V_BINS, NUM_BINS, true );
	cudaFree(bin_counts_d);
}
...
void FBP()
{
	...
	if( WRITE_FBP_IMAGE )
	{
		...
		array_2_disk( fbp_image, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, FBP_image_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );	
	}
	...
	if( IMPORT_FILTERED_FBP)
	{
		...
		sprintf(IMPORT_FBP_PATH,"%s%s/%s%d%s", parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, IMPORT_FBP_FILENAME, 2*FBP_MEDIAN_RADIUS+1,".bin" );
		import_image( image, IMPORT_FBP_PATH );
		FBP_image_h = image;
		char fbp_after[]={"FBP_after"};
		array_2_disk( fbp_after, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, image, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );
	}
	else if( AVG_FILTER_FBP )
	{
		...
		if( WRITE_AVG_FBP )
		{
			...
			array_2_disk( fbp_image_filter, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, FBP_image_filtered_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );
	...
	else if( MEDIAN_FILTER_FBP )
	{
		...
		if( WRITE_MEDIAN_FBP )
		{
			...
			array_2_disk( fbp_median_filter, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, FBP_median_filtered_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );
			//FBP_image_h = FBP_image_filtered_h;
		}
		cudaFree(FBP_image_filtered_d);
	}
...
void FBP_image_2_hull()
{
	...
	if( WRITE_FBP_HULL ) {
		char x_fbp[]={"x_FBP"};
		array_2_disk( x_fbp, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, FBP_hull_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );}
...
__global__ void MSC_edge_detection_GPU( int* MSC_counts )
{
	...
	if( max_difference>MSC_DIFF_THRESH || MSC_counts[voxel]>MSC_DIFF_THRESH )
		...
		MSC_counts[voxel] = 1;
	if( pow((double)x, 2.0) + pow((double)y, 2.0) >= pow((double)RECON_CYL_RADIUS - (double) max(VOXEL_WIDTH, VOXEL_HEIGHT)/2.0, 2.0 ) )
		MSC_counts[voxel] = 0;
...
void SM_edge_detection_2()
{
	...
	array_2_disk(sm_counts, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, SM_counts_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, false );
	...
	if( WRITE_SM_HULL ) {
	  char x_sm[]={"x_SM"};
		array_2_disk(x_sm, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, SM_counts_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );}
	if( MLP_HULL != SM_HULL)
		free(SM_counts_h);	
}
...
void hull_detection_finish()
{
	if( SC_ON )
	{
		...
		if( WRITE_SC_HULL )
		{
			...
			char x_sc[]={"x_SC"};
			array_2_disk(x_sc, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, SC_hull_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );
		}
		if( MLP_HULL != SC_HULL )
			free( SC_hull_h );
		cudaFree(SC_hull_d);
	...
	if( MSC_ON )
	{
		...
		{		
			...
			array_2_disk(msc_counts, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, MSC_counts_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );	
		}
		if( WRITE_MSC_HULL || (MLP_HULL == MSC_HULL) )
		{
			...
			if( WRITE_MSC_HULL )
			{
				...
				char x_msc[]={"x_MSC"};
				array_2_disk(x_msc, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, MSC_counts_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );	
			}
	...
	if( SM_ON )
	{
		...
		{		
			...
			array_2_disk(sm_counts, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, SM_counts_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );	
		}
...
void hull_selection()
{
	...
	switch( MLP_HULL )
	{
		...
		case MSC_HULL : for (int i = 0 ; i < NUM_VOXELS; i++) 
					{
					  if( MSC_counts_h[i] == 1 ) 
						hull_h[i] = true;
					  else 
						hull_h[i] = false;} 
						break;//std::transform( MSC_counts_h, MSC_counts_h + NUM_VOXELS, MSC_counts_h, hull_h, std::logical_or<int> () );		break;
	...
	if( WRITE_X_HULL )
	{
		...
		char hull[]={"hull"};
		array_2_disk(hull, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, hull_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );
	}
	...
	if( AVG_FILTER_HULL )
	{
		...
		if( WRITE_FILTERED_HULL )
		{
			...
			array_2_disk( hull_filter, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, hull_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );
		}
	}
...
void write_MLP_endpoints()
{
	...
	sprintf(endpoints_filename, "%s%s/%s_r=%d.bin", parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, MLP_ENDPOINTS_FILENAME, HULL_FILTER_RADIUS );
...
void reconstruction_cuts_allocations( const int num_histories)
{
	unsigned int size_floats		= sizeof(float) * num_histories;
	unsigned int size_ints			= sizeof(int) * num_histories;
	unsigned int size_bool			= sizeof(bool) * parameters_h->MAX_ENDPOINTS_HISTORIES;

	intersected_hull_h = (bool*)calloc( parameters_h->MAX_ENDPOINTS_HISTORIES, sizeof(bool) );
...
void reconstruction_cuts_deallocations()
{
	puts("MLP endpoints GPU memory deallocation...");	
	cudaFree( x_entry_d);
	cudaFree( y_entry_d);
	cudaFree( z_entry_d);
	cudaFree( x_exit_d);
...
void reconstruction_cuts_full_tx( const int num_histories )
{
	...
	{	    
		...
		( 
			...
			x_exit_d, y_exit_d, z_exit_d, xy_exit_angle_d, xz_exit_angle_d, i, num_histories, parameters_d
		);		
		
		cudaStatus = cudaGetLastError();
		if (cudaStatus != cudaSuccess) 
			printf("Error: %s\n", cudaGetErrorString(cudaStatus));	
		i += parameters_h->MAX_ENDPOINTS_HISTORIES;	  
	}  
...
{
  	printf("num_hist: %d\n", num_histories);
	//reconstruction_cuts_partial_tx_allocations(num_histories);
	...
	//reconstruction_cuts_partial_tx_device_2_host(start_position, num_histories);
	...
	puts("Free host/GPU memory...");
	//free(intersected_hull_h); 
	//reconstruction_cuts_partial_tx_deallocations();
	reconstruction_cuts_deallocations();
}
...
void reconstruction_cuts_full_tx_nobool( const int num_histories )
{
	...
	//reconstruction_cuts_full_tx_device_2_host_nobool( num_histories );
...
{ 
	printf("num_hist: %d\n", num_histories);
	//reconstruction_cuts_partial_tx_allocations_nobool(num_histories);
	//reconstruction_cuts_partial_tx_host_2_device_nobool( start_position, num_histories);
	...
	//reconstruction_cuts_partial_tx_device_2_host_nobool(start_position, num_histories);
	reconstruction_cuts_device_2_host_nobool(start_position, num_histories);
			
	puts("Vectors manupilating...");	
	for( int i = 0; i < num_histories; i++ ) 
	{    
		if( first_MLP_voxel_vector[i + start_position] == NUM_VOXELS + 1 ) 
		{
			first_MLP_voxel_vector[reconstruction_histories] = first_MLP_voxel_vector[i + start_position];
			data_shift_vectors( i + start_position, reconstruction_histories );
			reconstruction_histories++;
	...
	puts("Free host/GPU memory...");
	//free(intersected_hull_h); 
	//reconstruction_cuts_partial_tx_deallocations_nobool();
	reconstruction_cuts_deallocations_nobool();
}
		...
		{	
			...
			{		  
				...
				find_MLP_path_GPU_tabulated(x, WEPL[proton_id], first_MLP_voxel[proton_id] ,x_entry[proton_id], y_entry[proton_id], z_entry[proton_id], x_exit[proton_id], y_exit[proton_id], z_exit[proton_id], xy_entry_angle[proton_id],
						      xz_entry_angle[proton_id], xy_exit_angle[proton_id], xz_exit_angle[proton_id], param_d->LAMBDA, a_i, update_value_history, number_of_intersections,
...
__global__ void image_update_GPU (float* x, float* x_update, unsigned int* voxel_intersections, parameters* param_d) 
{
...
void DROP_full_tx(const int num_histories)
{
	...
	{	    
		...
		if( WRITE_X_KI ) 
		{
			...
			array_2_disk(iterate_filename, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, x_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true ); 
		}
	}
	DROP_deallocations();
}
	...
	for(int iteration = 1; iteration <= parameters_h->ITERATIONS ; ++iteration) {
		...
		while ( remaining_histories > 0 ) {
			...
			block_update_GPU_tabulated<<< num_blocks, parameters_h->HISTORIES_PER_BLOCK >>>( x_d, x_entry_d, y_entry_d, z_entry_d, xy_entry_angle_d, xz_entry_angle_d, x_exit_d, y_exit_d, z_exit_d,  xy_exit_angle_d, 
					 xz_exit_angle_d, WEPL_d, first_MLP_voxel_d, x_update_d, S_d, start_position, start_position + histories_2_process,
					 sin_table_d, cos_table_d, scattering_table_d, poly_1_2_d, poly_2_3_d, poly_3_4_d, poly_2_6_d, poly_3_12_d, parameters_d);
...
{  
	...
	{
		// intersected_hull_h/d used to identify protons hitting/missing hull
		...
		// first_MLP_voxel_vector used to identify protons hitting/missing hull
	...
	// Reconstruction_cuts with partial data transfers and GPU arrays allocated/freed each kernel launch
	...
	// Reconstruction_cuts with partial data transfers and preallocated and reused GPU arrays
...
void create_hull_image_hybrid()
{
	...
	if( WRITE_X_0 ) {
		char x_k0[]={"x_k0"};
		array_2_disk(x_k0, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, x_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );}
}
...
void define_initial_iterate()
{
	...
	switch( X_0 )
	{
		...
		case HYBRID		: std::transform(FBP_image_h, FBP_image_h + NUM_VOXELS, hull_h, x_h, std::multiplies<float>() );		break;
		case IMPORT		: sprintf( INPUT_ITERATE_PATH, "%s%s/%s", parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, INPUT_ITERATE_FILENAME );
	...
	if( WRITE_X_0 ) {
		char x_0[]={"x_0"};
		array_2_disk(x_0, parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, x_h, COLUMNS, ROWS, SLICES, NUM_VOXELS, true );}
	//cudaMalloc((void**) &x_d, SIZE_IMAGE_FLOAT );
	//cudaMemcpy( x_d, x_h, SIZE_IMAGE_FLOAT, cudaMemcpyHostToDevice );
}
...
double radial_lambda( double radius_squared )
{
	...
	return parameters_h->LAMBDA * ( 1 - DECAY_FACTOR * radius_squared );
...
void image_reconstruction()
{
	...
	*/
	...
	sprintf(MLP_filename, "%s%s/%s_r=%d.bin", parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, MLP_PATHS_FILENAME, HULL_FILTER_RADIUS );
	...
	sprintf(MLP_error_filename, "%s%s/%s_r=%d.bin", parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, MLP_PATHS_ERROR_FILENAME, HULL_FILTER_RADIUS );
	FILE* read_MLP_paths;	
...
template<typename X> void update_iterate2( double bi, double mean_chord_length, X*& x_k, unsigned int*& voxels_intersected, unsigned int num_intersections )
{
	...
	double scale_factor = parameters_h->LAMBDA * ai_multiplier * mean_chord_length;
...
void binary_2_ASCII()
{
	...
	while( start_file_num != NUM_FILES )
	{
		...
		read_data_chunk( histories_2_process, start_file_num, end_file_num );
		sprintf( filename, "%s%s/%s%s%d%s", parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, INPUT_BASE_NAME, "_", gantry_angle_h[0], ".txt" );
...
template<typename T> void bins_2_disk( const char* filename_base, const std::vector<int>& bin_numbers, const std::vector<T>& data, const BIN_ANALYSIS_TYPE type, const BIN_ANALYSIS_FOR which_bins, const BIN_ORGANIZATION bin_order, ... )
{
	...
	for( int angular_bin = 0; angular_bin < num_angles; angular_bin++)
	{
		...
		sprintf( filename, "%s%s/%s_%03d%s", parameters_h->OUTPUT_DIRECTORY, parameters_h->OUTPUT_FOLDER, filename_base, angle, ".txt" );
		output_file = fopen (filename, "w");
